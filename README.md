...
Workflow、data processor、context routing、agent responsibilities、validation gates、ReportRenderer、CLI/API、CI/test gates の詳細は [System details](docs/system/earnings-review-system.md) に分離しています。環境ごとに変える値は [.env.example](.env.example) を起点にし、各環境変数の意図と default は同文書の `Configuration / Environment Variables` を参照してください。
